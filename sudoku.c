 ...
 */
void printd_init(dprint_t *dprint)
{
    dprint->log_filename = "./logs/sudoku_log";
    dprint->log_fd       = NULL;
    // Fixme:  Magic number.  Disables all assigned debug flags.
...
input_values(int argc, char **argv, game_t *game)
{
    ...
    while (fgets(input_buf, MAX_LINE_SIZE, input_fd)) {
        ...
        for (col=0; col < NUM_HORIZ_CELLS; col++) {
            ...
            set_cell_value_board(game->board_curr, num_rows_read, col, col_val);
        }
        num_rows_read++;
    }
#endif
